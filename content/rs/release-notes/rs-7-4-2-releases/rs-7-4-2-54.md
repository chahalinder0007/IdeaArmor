...
- New clusters fully support IPv6, including for internal traffic. See [Enable IPv6 for internal traffic]({{<relref "/rs/networking/multi-ip-ipv6#enable-ipv6-for-internal-traffic">}}) for details.
...
- Improved `actions` REST API:
    ...
    - Improved `progress` field returned by [`GET /v1/actions`]({{<relref "/rs/references/rest-api/requests/actions">}}) requests.
...
- Type of `progress` field returned by [`GET /v1/actions`]({{<relref "/rs/references/rest-api/requests/actions">}}) request modified from `integer` to `float`.
